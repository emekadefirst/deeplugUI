...
export const CountrySelector = React.memo(({ countries, selected, search, onSearch, onSelect }: Props) => {
    ...
    return (
        <div className="space-y-2" ref={ref}>
            <label className="block text-sm font-bold text-[#2c3e5e]">Select Country *</label>
                ...
                >
                    <div className="flex items-center gap-3">
                        ...
                        <span className={selected ? 'text-[#2c3e5e] font-bold' : 'text-gray-400'}>
                            {selected ? `${selected.name} (+${selected.cc})` : 'Choose a country...'}
                        </span>
                    </div>
                        ...
                        <div className="max-h-60 overflow-y-auto">
                            {countries.length === 0 ? (
                                ...
                                countries.map((country) => (
                                    ...
                                    >
                                        <div>
                                            <p className="font-bold text-[#2c3e5e] group-hover:text-[#2c3e5e]">{country.name}</p>
                                            <p className="text-[10px] text-gray-400 uppercase tracking-widest">+{country.cc}</p>
                                        </div>
                                        {selected?.ID === country.ID && <Check className="w-4 h-4 text-[#2c3e5e]" />}
                                    </button>
                                ))
                            )}
                        </div>
